...
class Model(object):
	def __init__(self, filename):
		...
		# Camera helpers (populated externally when needed)
		self.focusOffset = 0.0
		self.defaultOrbitDistance = 5.0
	...
	def _upload_texture_surface(self, surface):
		if not pygame.get_init():
			pygame.init()

		has_alpha = surface.get_masks()[3] != 0 if len(surface.get_masks()) == 4 else False
		format_str = "RGBA" if has_alpha else "RGB"
		gl_format = GL_RGBA if has_alpha else GL_RGB

		texture_data = pygame.image.tostring(surface, format_str, True)
	...
	def AddTexture(self, filename):
		...
		if textureSurface.get_flags() & pygame.SRCALPHA:
			textureSurface = textureSurface.convert_alpha()
		else:
			textureSurface = textureSurface.convert()
		return self._upload_texture_surface(textureSurface)


	def AddSolidColorTexture(self, color=(255, 255, 255, 255), size=4):
		if not pygame.get_init():
			pygame.init()
		...
		surface.fill(color)
		return self._upload_texture_surface(surface)
